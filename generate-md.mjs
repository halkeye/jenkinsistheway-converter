...
const IGNORED_POSTS = [
	'd4science-amps-up-their-scientific-research-platform-with-ci-cd-powered-by-jenkins', // different format
	'jenkins-case-study-preply', // same as above format
	'to-build-a-scalable-ci-cd-orchestration-platform', // is fine, but needs to be ignored until things are appeneded properly
	'to-automate-the-work', // is fine, but needs to be ignored until we capture remaining/appended properly
	'to-build-ci-cd-that-fits-advanced-and-unique-use-cases', // is fine, but needs to be ignored until we capture remaining/appended properly
	'to-automate-create-and-execute', // same
...
for (const item of data) {
	...
	if (item.post_type === 'post') {
		...
		if (IGNORED_POSTS.includes(itemKey)) {
			// FIXME - Skipping cause its a different format
			continue;
		}
		...
		if (item?.category?._ === "map") {
			...
			const mapKey = item.frontmatter['story link'].replace('https://jenkinsistheway.io/user-story/', '').replace(/\/$/, '')
			items[mapKey] = {
				...(items[mapKey] || {}),
				map: {
		...
		story.tmp.md = story.tmp.md.replace(/\\\*\\\*SUMMARY:\s+(.*?)\\\*\\\*\s*$/m, '**Summary:** $1')
		story.tmp.md = story.tmp.md.replace(/\\\*\\\*SOLUTION:\s+(.*?)\\\*\\\*\s*$/m, '**Solution:** $1')

		story.tmp.md = story.tmp.md.replace(/\*\*\.\s*$/m, '.**') // period after bold gets moved inside
		for (const header of Object.keys(FIELDS)) {
			story.tmp.md = story.tmp.md.replace(
				new RegExp(`^\\*\\*${escapeRegExp(header)}:\\s+(.*?)\\*\\*\\s*$`, 'm'),
				`**${header}:** $1`
			)
			story.tmp.md = story.tmp.md.replace(
				new RegExp(`^${escapeRegExp(header)}:\\s+`, 'm'),
				`**${header}:** `
			)
		}

		story.tmp.md = story.tmp.md.replace(/^\*\*Community Support:\s+(.*?)\*\*\s*$/m, '**Community Support:** $1')
		story.tmp.md = story.tmp.md.replace(/^Community Support:\s+(.*?)\s*$/m, '**Community Support:** $1')
		story.tmp.md = story.tmp.md.replace(/^\*\*Background:\s+(.*?)\*\*\s*$/m, '**Background:** $1')
		story.tmp.md = story.tmp.md.replace(/^\*\*Organization:\s+(.*?)\*\*\s*$/m, '**Organization:** $1')
		story.tmp.md = story.tmp.md.replace(/^\*\*Programming Language:\s+(.*?)\*\*\s*$/m, '**Programming Language:** $1')
		story.tmp.md = story.tmp.md.replace(/^\*\*SUMMARY:\s+(.*?)\*\*\s*$/m, '**Summary:** $1')
		story.tmp.md = story.tmp.md.replace(/^\*\*CHALLENGE:\s+(.*?)\*\*\s*$/m, '**Challenge:** $1')
		story.tmp.md = story.tmp.md.replace(/^\*\*SOLUTION:\s+(.*?)\*\*\s*$/m, '**Solution:** $1')
		story.tmp.md = story.tmp.md.replace(/^\*\*RESULTS:\s+(.*?)\*\*\s*$/m, '**Results:** $1')
				...
				return (tree, _file) => {
					tree.children = tree.children.filter(child => {
						if (child.type === 'heading') {
							const line = toMarkdown({type: 'paragraph', children: child.children}, {emphasis: ''}).trim();
							if (child.depth == 1) {
					...
					for (let i = 0; i < tree.children.length; i++) {
						...
						if (child.type === 'paragraph') {
							...
							if (child.children[0].type === 'strong') {
								...
								let line = toMarkdown({type: 'paragraph', children: child.children.slice(1)}, {emphasis: '', strong: ''}).trim().replace(/^\*+(.*?)\*+$/, '$1').trim();
								...
								if (FIELDS[header]) {
									...
									if (field.type === 'singular') {
										...
										if (!story[field.section][key]) {
											...
											continue;
										} else if (story[field.section][key] && story[field.section][key].trim() == line.trim()) {
											nodesToRemove.push(child);
											continue;
										}
...
(function () {
	const stories = Object.values(items).filter(item => item?.tmp?.md);
	if (stories.length) {
